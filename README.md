...
- Initially created to setup WIF account
- When run specify a function as an arg

```bash
# view roles assigned to service account
./setup_gcp_wif.sh get_service_account_roles
```

To add a new role create a function like this. Replacing `++ROLE_NAME++` and `++ROLE++`
...
# removes role to account
remove_role_++ROLE_NAME++() {
  display_service_account
